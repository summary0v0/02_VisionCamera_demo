...
class MainHardwareIntegration:
    ...
    def init_test_ui(self):
        ...
        try:
            # 检查是否存在日志控件
            if not hasattr(self.main_window, 'logPlainTextEdit'):
                logging.warning("未找到 logPlainTextEdit 控件，无法添加测试按钮")
                return

            log_widget = self.main_window.logPlainTextEdit
            parent_widget = log_widget.parentWidget()
            
            if not parent_widget or not parent_widget.layout():
                logging.warning("日志控件没有父布局，无法插入测试按钮")
                return

            # 创建按钮容器 widget
            test_container = QWidget()
            layout = QHBoxLayout(test_container)
            layout.setContentsMargins(0, 5, 0, 5) # 设置边距

            # 创建按钮
            btn_edge_test = QPushButton("🛠️ 侧磨机测试")
            btn_flip_test = QPushButton("🔄 翻板机测试")
            btn_sort_test = QPushButton("🔀 分拣机测试")
            btn_all_test = QPushButton("🚀 全部测试")
            ...
            """
            for btn in [btn_edge_test, btn_flip_test, btn_sort_test, btn_all_test]:
                btn.setStyleSheet(btn_style)
                # 设置手型光标 (兼容处理)
                if 'PyQt5.QtCore' in sys.modules:
                    btn.setCursor(sys.modules['PyQt5.QtCore'].Qt.PointingHandCursor)
                layout.addWidget(btn)

            # 绑定点击事件
            btn_edge_test.clicked.connect(self.test_action_edge)
            btn_flip_test.clicked.connect(self.test_action_flipper)
            btn_sort_test.clicked.connect(self.test_action_sorter)
            btn_all_test.clicked.connect(self.test_action_all)

            # 将容器插入到日志控件上方
            parent_layout = parent_widget.layout()
            
            # 获取日志控件在布局中的索引
            index = parent_layout.indexOf(log_widget)
            if index != -1:
                # 插入到日志控件之前 (即上方)
                parent_layout.insertWidget(index, test_container)
                self.hardware_ui.log_message("已加载硬件测试控制面板")
            else:
                # 如果找不到索引，就添加到末尾（备选方案）
                parent_layout.addWidget(test_container)

        except Exception as e:
            logging.error(f"初始化测试按钮UI失败: {e}")

    # ==================== 测试按钮响应动作 ====================

    def test_action_edge(self):
        """侧磨机参数设置示例：手动下发加工任务到队列"""
        try:
            self.hardware_ui.log_message("--- 开始侧磨机模拟测试 (下发任务到队列) ---")
            
            # 1. 准备工艺数据（实际开发中可改为从 UI 输入框获取）
            machine_name = "磨边机1号"  # 必须与配置文件中的名称一致
            process_type = 1  # 工艺类型 (例如：1-直边)
            thickness = 18.0  # 材料厚度
            angle = 45.0  # 斜边角度

            # 2. 检查控制器方法是否存在
            if hasattr(self.hardware_controller, 'add_edge_task'):
                # 3. 调用 HardwareController 的 add_edge_task 函数
                # 该函数会将任务放入对应的 self.edge_queues[machine_name] 队列中
                self.hardware_controller.add_edge_task(machine_name, process_type, thickness, angle)

                # 4. 界面日志反馈
                self.hardware_ui.log_message(f"✅ 手动下发任务成功: {machine_name} | 工艺:{process_type} | 厚度:{thickness}| 斜边角度:{angle}")
            else:
                self.hardware_ui.log_message("❌ 错误: HardwareController 中未找到 add_edge_task 方法")

        except Exception as e:
            error_msg = f"手动下发工艺异常: {e}"
            self.hardware_ui.log_message(f"❌ {error_msg}")
            logging.error(error_msg)
            traceback.print_exc()

    def test_action_flipper(self):
        """翻板机测试动作"""
        self.hardware_ui.log_message("--- 开始翻板机模拟测试 ---")
        # 模拟翻板机任务请求
        self.handle_task_request('flipper', '模拟_翻板机')

    def test_action_sorter(self):
        """分拣机测试动作"""
        self.hardware_ui.log_message("--- 开始分拣机模拟测试 ---")
        # 模拟分拣机任务请求
        self.handle_task_request('sorter', '模拟_分拣机')

    def test_action_all(self):
        """全部测试动作"""
        self.hardware_ui.log_message("=== 开始全流程模拟测试 ===")
        # 依次触发，稍微加点延时让日志看得清楚
        QTimer.singleShot(100, lambda: self.test_action_edge())
        QTimer.singleShot(1000, lambda: self.test_action_flipper())
        QTimer.singleShot(2000, lambda: self.test_action_sorter())
    ...
    def handle_scan_flag(self, device_type: str, device_name: str):
        ...
        Args:
            ...
            device_name: 设备名称
        """
        self.hardware_ui.log_message(f"🔍 检测到 {device_name} 扫码标志 ({device_type})")
        
        # 如果是侧磨机，通常这里会触发扫码枪读取，但如果是手动测试，
        # 我们假设已经获取了信息并处理
    ...
    def process_edge_grinder_scan(self, machine_name: str):
        """
        处理侧磨机扫码事件 (保留旧方法，仅作兼容)
        实际业务中，建议使用 add_edge_task 配合 HardwareController 内部的 scan_flag 监控
        """
        try:
            # 示例：发送默认工艺参数
            process_type = 1  # 直边
    ...
    def process_flipper_task(self, device_name: str):
        ...
        try:
            # 示例：简单的业务逻辑
            need_flip = True  # 模拟需要翻板
            self.hardware_ui.log_message(f"  -> 决策: {'需要翻板' if need_flip else '直通'}")
            ...
            if need_flip:
                ...
                if success:
                    ...
                    self.hardware_ui.log_message(f"⚠️ {device_name} 执行翻板指令未发送 (未连接?)")
            else:
                success = self.hardware_controller.flipper_pass_command()
                if success:
                    self.hardware_ui.log_message(f"✅ {device_name} 执行通过指令成功")
                else:
                    self.hardware_ui.log_message(f"⚠️ {device_name} 执行通过指令未发送 (未连接?)")
    ...
    def process_sorter_task(self, device_name: str):
        ...
        try:
            # 示例：简单的分拣逻辑
            target_station = 1  # 模拟分拣到1号口
            self.hardware_ui.log_message(f"  -> 决策: 分拣至 {target_station} 号工位")
            ...
            if success:
                ...
                self.hardware_ui.log_message(f"⚠️ {device_name} 分拣指令未发送 (未连接?)")
