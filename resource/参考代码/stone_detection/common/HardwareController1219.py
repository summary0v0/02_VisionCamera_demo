    ...
    def __init__(self, config_path: str = "./configs/hardware_config.txt"):
        ...
        self.EDGE_REGISTERS_TEMPLATE = {
            ...
            'process_type': {
                ...
                'description': '工艺类型',
                'read_only': False,
                'values': {1: '直边', 2: '圆边', 3: '大斜边', 4: '背切', 5: '保留'}
            },
            'material_thickness': {
                'type': 'real',
                'description': '材料厚度',
...
class HardwareController(QObject):
    ...
    def __init__(self):
        ...
        # ==================== 扫描枪 ====================
        self.scanner_controller = ScannerController(scanner_configs=self.config.SCANNERS)
        # 启动处理线程
    ...
    def _process_before_processing_queue(self):
        ...
        while True:
            ...
            time.sleep(5)
            print("处理加工前扫描枪队列。")
            composite_id, scale_url, processing_type, thickness = "25-5-181A177", "https://lsmc-oss.oss-cn-shanghai.aliyuncs.com/middleware/lsmc/1927548980971450370/cropMap/cell/cbbc5501-cd02-488e-8bfc-125e5d7b514c.png", "4321", float(18.0)
            ...
            for i, ch in enumerate(processing_type):
                if self.edge_connected.get(directions[i]) and process_map.get(ch):
                    print(f"第一次 厚度thickness={thickness}, 它的类型为type={type(thickness)}")
                    self.edge_queues[directions[i]].put((process_map[ch], thickness, 0.0))
                    print(
                        f"[磨边机] {directions[i]}, 投递侧磨机任务: composite_id={composite_id}, 工艺={process_map[ch]}, 厚度={thickness}",
                        flush=True)
                    print(f"第二次 厚度thickness={thickness}, 它的类型为type={type(thickness)}")
                elif not self.edge_connected.get(directions[i]):
                    print(f"[磨边机] {directions[i]} 未连接，跳过任务")
    ...
    def _write_edge_real_register(self, machine_name: str, register_name: str, value: float) -> bool:
        ...
        register_info = machine_config['registers'][register_name]
        if register_info.get('type') != 'float':
            logging.error(f"寄存器 {register_name} 不是float类型")
            return False
        ...
        try:
            # --- 增加一行强制转换，确保 value 是 float 而不是 int ---
            f_value = float(value)

            # 将浮点数转换为两个16位寄存器
            raw_bytes = struct.pack('>f', f_value)
            reg1, reg2 = struct.unpack('>HH', raw_bytes)

            # 写入两个寄存器
            result = client.write_registers(address=address, values=[reg1, reg2], slave=slave_id)

            if not result.isError():
                logging.info(f"写入 {machine_name}.{register_name}: {f_value}")
                print(f"浮点数写入成功：", f_value,type(f_value))
                return True
            else:
                logging.error(f"写入 {machine_name}.{register_name} 失败: {result}")
                self.error_occurred.emit('edge_grinder', machine_name, f"写入寄存器失败: {result}")
...
class ScannerController:
    def __init__(self, scanner_configs: Dict[str, Dict] = None):
